...
- **Existing Codex CLI login**: `/login` includes the login-only alias `openai-codex-cli`, which authenticates the existing `openai-codex` model provider. It asks the installed official `codex app-server` for its current short-lived ChatGPT bearer through `getAuthStatus` and stores only that access token, account metadata, and a source marker; OMP never reads or stores Codex's real refresh token. When the bearer nears expiry, Codex app-server remains the sole refresh owner and returns the replacement bearer. This bridge supports Codex file or keyring credential storage but is intentionally local-only: remote auth-broker uploads fail closed because the broker host cannot safely act for the source machine. If the official Codex login switches accounts, refresh also fails closed until the user explicitly selects this login entry again to confirm the new binding.
