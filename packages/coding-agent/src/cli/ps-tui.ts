...
class PsTopComponent implements Component {
	...
	async #act(verb: "stop" | "kill" | "restart"): Promise<void> {
		...
		const action = t(`ps.action.${verb}`);
		this.#setStatus(chalk.yellow(t("ps.tui.actionPending", { action, name })));
		try {
			const client = await this.#client(entry.scope);
			if (!client) throw new Error(t("ps.tui.scopeUnavailable"));
			...
			if (result.op !== "restart" && result.op !== "stop")
				throw new Error(t("ps.unexpectedResponse", { response: result.op }));
			this.#setStatus(
				chalk.green(
					`${t(verb === "restart" ? "ps.verb.restarted" : verb === "kill" ? "ps.verb.killed" : "ps.verb.stopped")} ${daemonLabel(result.daemon)}`,
				),
			);
			void this.#refresh();
		} catch (error) {
	...
	async #openInfo(): Promise<void> {
		...
		try {
			...
			if (result.op !== "describe") throw new Error(t("ps.unexpectedResponse", { response: result.op }));
	...
	#openLogs(): void {
		...
		const poll = async (): Promise<void> => {
			...
			try {
				...
				if (result.op !== "logs") throw new Error(t("ps.unexpectedResponse", { response: result.op }));
	...
	handleInput(data: string): void {
		...
		if (matchesKey(data, "up") || data === "k") this.#moveSelection(-1);
			...
			this.#all = !this.#all;
			this.#setStatus(chalk.dim(t(this.#all ? "ps.tui.showingAll" : "ps.tui.showingCurrent")));
			void this.#refresh();
	...
	#header(width: number, title: string): string {
		const age = this.#lastRefresh
			? t("ps.tui.updatedAgo", { duration: formatDuration(Date.now() - this.#lastRefresh) })
			: t("ps.tui.updating");
	...
	#renderTable(width: number, height: number): string[] {
		const scopesLabel = t("ps.tui.summary", {
			processes: t(this.#flat.length === 1 ? "ps.tui.processOne" : "ps.tui.processMany", {
				count: this.#flat.length,
			}),
			scopes: t(this.#reports.length === 1 ? "ps.tui.scopeOne" : "ps.tui.scopeMany", {
				count: this.#reports.length,
			}),
			scopeMode: chalk.dim(t(this.#all ? "ps.tui.all" : "ps.tui.current")),
		});
		const panelHeader = this.#header(width, scopesLabel);
		const footer = this.#footer(width, t("ps.tui.hints"));
		...
		const widths = tableHeadings.map((title, column) =>
		...
		for (const report of this.#reports) {
			...
			if (report.daemons.length === 0) {
				body.push({ text: chalk.dim(`   ${t("ps.noProcesses")}`) });
			} else {
				body.push({ text: chalk.dim(renderRow(tableHeadings)) });
		...
		if (body.length === 0) body.push({ text: chalk.dim(` ${t("ps.noScopes")}`) });
	...
	#renderInfo(width: number, height: number): string[] {
		...
		const header = this.#header(width, t("ps.tui.processInfo"));
		const footer = this.#footer(width, t("ps.tui.backHints"));
		const lines = [header, ""];
		if (info) {
			const daemon = info.daemon;
			lines.push(` ${chalk.bold(daemonLabel(daemon))}`);
			lines.push("");
			lines.push(`   ${t("ps.info.command")} ${collapseCommand(formatCommand(info.spec))}`);
			lines.push(`   ${t("ps.info.cwd")} ${info.spec.cwd}`);
			if (!TERMINAL_STATES[daemon.state])
				lines.push(`   ${t("ps.info.uptime")} ${formatDuration(Date.now() - daemon.startedAt)}`);
			if (daemon.exitReason) lines.push(`   ${t("ps.info.exit")} ${daemon.exitReason}`);
			lines.push(
				`   ${t("ps.info.restarts")} ${daemon.restartCount} (${t("ps.info.policy", { policy: info.spec.restart })})`,
			);
			lines.push(
				`   ${t("ps.info.pty", { value: info.spec.pty })}  ${t("ps.info.persist", { value: info.spec.persist })}  ${t("ps.info.detached", { value: info.spec.detached })}`,
			);
			lines.push(`   ${t("ps.info.owner", { owner: daemon.owner ?? "-" })}`);
		} else {
			lines.push(chalk.dim(` ${t("ps.tui.loading")}`));
		}
	...
	#renderLogs(width: number, height: number): string[] {
		...
		const header = this.#header(
			width,
			t("ps.tui.logsTitle", { name: chalk.bold(name) }) +
				(this.#logsState ? chalk.dim(` · ${this.#logsState}`) : ""),
		);
		const footer = this.#footer(width, t("ps.tui.logsHints"));
		const bodyHeight = height - 1 - footer.length;
