...
export function scopeHeader(scope: PsScope): string {
	const name = chalk.bold(
		scope.kind === "global"
			? (scope.service ?? path.basename(scope.runtimeDir))
			: (scope.projectDir ?? path.basename(scope.runtimeDir)),
	);
	const label = t(scope.kind === "global" ? "ps.scope.global" : "ps.scope.project", { name });
	const broker =
		scope.brokerPid !== undefined
			? chalk.green(t("ps.scope.brokerPid", { pid: scope.brokerPid }))
			: chalk.dim(t("ps.scope.brokerStopped"));
