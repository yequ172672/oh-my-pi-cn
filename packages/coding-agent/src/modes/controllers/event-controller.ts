...
export class EventController {
	...
	#maintenanceEscHint(): string {
		return this.ctx.focusedAgentId ? "" : t("compaction.cancelHint");
	}
		...
		const reasonText =
			event.reason === "overflow"
				...
				: event.reason === "incomplete"
					? t("compaction.reason.incomplete")
					: event.reason === "idle"
						? t("compaction.reason.idle")
						: "";
		const actionLabel =
			event.action === "remote"
				? t("compaction.remote")
	...
	async #handleAutoCompactionEnd(event: Extract<AgentSessionEvent, { type: "auto_compaction_end" }>): Promise<void> {
		...
		if (event.aborted) {
			...
			this.ctx.showWarning(t("compaction.remoteFailed"));
		} else {
