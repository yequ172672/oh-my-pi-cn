...
export class CleansePanelComponent extends OverlayPanel implements CleanseStatusBoard {
	...
	#footerLine(): string {
		switch (this.#outcome) {
			case undefined:
				return theme.fg("muted", t("cleanse.panel.cancelHint"));
			case "clean":
				return theme.fg("success", `${theme.status.success} ${t("cleanse.panel.clean")}`);
			case "unresolved":
				return theme.fg("warning", `${theme.status.warning} ${t("cleanse.panel.unresolved")}`);
			case "unsupported":
				return theme.fg("warning", `${theme.status.warning} ${t("cleanse.panel.unsupported")}`);
			case "cancelled":
				return theme.fg("warning", `${theme.status.warning} ${t("cleanse.panel.cancelled")}`);
			case "error":
				return theme.fg("error", `${theme.status.error} ${t("cleanse.panel.error")}`);
		}
	}
}
