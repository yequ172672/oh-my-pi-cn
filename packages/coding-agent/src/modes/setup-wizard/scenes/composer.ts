...
class ComposerSceneController implements SetupSceneController {
	title = localizeUiText("Choose composer shape");
	subtitle = localizeUiText("Pick the prompt and status line layout for your workflow.");
	...
	constructor(private readonly host: SetupSceneHost) {
		...
		this.#items = choices.map((choice, index) => ({
			value: choice.value,
			label: `${index + 1}  ${localizeUiText(choice.label)}`,
			description: choice.description ? localizeUiText(choice.description) : choice.description,
		}));
	...
	render(width: number, maxLines?: number): readonly string[] {
		...
		const lines = [
			theme.fg("muted", localizeUiText("Select a layout; live preview updates below. Press Enter to confirm.")),
			"",
		];
		...
		if (budget - lines.length - previewLines.length - 2 >= this.#items.length) {
			lines.push(theme.fg("muted", localizeUiText("Preview:")), ...previewLines, "");
		}
...
export const composerSetupScene: SetupScene = {
	id: "composer-shape",
	get title() {
		return localizeUiText("Choose composer shape");
	},
	minVersion: 2,
