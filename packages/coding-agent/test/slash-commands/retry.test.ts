...
describe("/retry dispatch (ACP)", () => {
	...
	it("reports retry start and no-op states in Chinese", async () => {
		setLanguage("zh-CN");
		const scheduled = acpRuntime({ retryResult: true });
		const noop = acpRuntime({ retryResult: false });

		await executeAcpBuiltinSlashCommand("/retry", scheduled.runtime);
		await executeAcpBuiltinSlashCommand("/retry", noop.runtime);

		expect(scheduled.output).toHaveBeenCalledWith("正在重试上一次失败的回合。");
		expect(noop.output).toHaveBeenCalledWith("没有可重试的内容。");
	});
